...
## Беговой режим (шагомер PWA)

- Web: страница `/run` (пункт «Бег» в меню), шаги считает акселерометр —
  `src/utils/stepCounter.ts` (DeviceMotion, пиковая детекция). Работает только
  при активном экране, поэтому страница держит Screen Wake Lock. Дистанция —
  шаги × 0.65 роста, калории — MET ≈ скорость км/ч.
- API: `POST /workouts/runs` сохраняет уже завершённую сессию (name «Бег»,
  поле `run: {steps, distanceM}` в WorkoutSession) + событие ленты и +5 XP.
